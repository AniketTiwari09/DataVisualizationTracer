...
if not st.session_state.logged_in:
    ...
    if st.session_state.logged_in:
        ...
        with st.expander("**Upload Own Files Section**", expanded=False):
            ...
            if submit_all_forms: 
                st.title("File History")
                for i, item in user_uploaded_cluster_data_files.items():
                    file_path, file_contents = save_uploaded_file_to_db(item, user_id, conn)
    ...
    if user_files:
        for i, file in enumerate(user_files):
            col_logic = col_logic = col1 if i % 2 == 0 else col2
            file_name = file[0]
            unique_key = f"{i}_{file_name}"  
            with col_logic:
                if st.button(file_name, key=unique_key):
